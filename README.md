...
**Important**: This plugin is currently in *alpha* stage. It notably lacks support for delayed payment methods (multibanco, bank transfers) and would need to be tested by users on the features already implemented.
...
**Important** : Ce plugin est actuellement en phase *alpha*. Il manque notamment le support des méthodes de paiement différées (multibanco, virements bancaires) et nécessiterait d'être testé par des utilisateurs sur les fonctionnalités déjà implémentées.
